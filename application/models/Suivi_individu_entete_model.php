...
class Suivi_individu_entete_model extends CI_Model
{
    protected $table = 'suivi_individu_entete';

	// TABLE CONCERNEE DANS LA BDD : suivi_individu_entete
	// Cette fonction ajoute un enregistrement dans la table
    public function add($suivi_individu_entete)
    {
        $this->db->set($this->_set($suivi_individu_entete))
                            ->insert($this->table);
    ...
    public function update($id, $suivi_individu_entete)
    {
        $this->db->set($this->_set($suivi_individu_entete))
                            ->where('id', (int) $id)
                            ->update($this->table);
    ...
    public function _set($suivi_individu_entete)
    {
       return array(
            'id_intervention'                  => $suivi_individu_entete['id_intervention'],                      
            'date_suivi'                       => $suivi_individu_entete['date_suivi'],                      
    ...
    public function findAllByProgramme($id_interventions)
    {
        $result =  $this->db->select('menage.id as id_individu,
                                        suivi_individu_entete.id as id,
                                        ...
                                        ')
                        ->from($this->table)
                        ->join('menage', 'menage.id = suivi_individu_entete.id_individu')
                    //    ->order_by('id')
    ...
    public function findAllByProgrammeAndIndividu($id_interventions,$id_individu)
    {
		$requete="select sm.id,sm.id_individu,i.nom,i.prenom,i.date_naissance,"
				...
				." from suivi_individu_entete as sm"
